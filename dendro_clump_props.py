...
clumptype_list = ["clump type"]


def clumptype_function(clumps, ncl):
	if clumps[ncl].is_leaf:
		...
		clumptype = "neither"
	clumptype_list.append(clumptype)
	return clumptype
...
def operations(clumps, rms_K, nsig, dpc, COtemp, ncl, velocity_range, arcsec_per_pix, brad):
	...
	current_clump = np.where(mask, COtemp, np.nan)
	mask3d = np.where(current_clump > nsig*rms_K, 1., np.nan)
	#Calculate moments 0 and 8 and mask below nsig
	mom0 = np.nansum(current_clump, axis=0) #total intensity K
	mom8 = np.nanmax(current_clump, axis=0) #peak intensity K
...
if __name__ == "__main__":
	...
	clumps = Dendrogram.load_from('dendro_dendrogram_ab6high_5sig.fits')
	...
	clumptype = 2 #all clumps from quickclump are type 2 clumps
	props_array = []
	for ncl in range(1,3):
#	for ncl in range(1, ncl_total):
#	for ncl in np.array([ncl_total]):
		print(ncl)
		print(clumptype_function(clumps, ncl))
